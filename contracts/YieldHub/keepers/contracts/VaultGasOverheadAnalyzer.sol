...
import "../interfaces/IYieldHubStrategyEthCall.sol";
...
contract VaultGasOverheadAnalyzer is ManageableUpgradeable, KeeperCompatibleInterface {
    ...
    IYieldHubRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        IYieldHubVault vaultToAnalyze = IYieldHubVault(vaults[currentIndex]);
    ...
    function _analyzeHarvest(IYieldHubVault vault) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        IYieldHubStrategyEthCall strategy = IYieldHubStrategyEthCall(address(IYieldHubVault(vault).strategy()));
    ...
    function _tryHarvest(IYieldHubStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function _tryOldHarvest(IYieldHubStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        uint256 gasBefore = gasleft();
