...
import "../interfaces/IYieldHubVault.sol";
import "../interfaces/IYieldHubStrategy.sol";
import "../interfaces/IYieldHubRegistry.sol";
import "../interfaces/IYieldHubHarvester.sol";
import "../interfaces/IUpkeepRefunder.sol";

import "../libraries/UpkeepLibrary.sol";

contract YieldHubHarvester is ManageableUpgradeable, IYieldHubHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    IYieldHubRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in YieldHubRegistry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = IYieldHubRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("YieldHubAutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IYieldHubVault vault = IYieldHubVault(vaultAddress_);
        IYieldHubStrategy strategy = IYieldHubStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from YieldHubRegistry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IYieldHubStrategy strategy = IYieldHubStrategy(IYieldHubVault(vault_).strategy());
