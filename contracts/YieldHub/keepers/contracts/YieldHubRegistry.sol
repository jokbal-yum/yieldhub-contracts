...
contract YieldHubRegistry is ManageableUpgradeable, IYieldHubRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        IYieldHubVault vault = IYieldHubVault(_vaultAddress);
        IYieldHubStrategy strat = _validateVault(vault);
    ...
    function _validateVault(IYieldHubVault _vault) internal view returns (IYieldHubStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (IYieldHubStrategy _strategy) {
            require(IYieldHubStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return IYieldHubStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(IYieldHubStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = IYieldHubStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (IYieldHubVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
