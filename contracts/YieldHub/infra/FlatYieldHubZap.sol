...
interface IYieldHubVaultV6 is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address);
}

contract YieldHubUniV2Zap {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IYieldHubVaultV6;
    ...
    function beefInETH (address yieldhubVault, uint256 tokenAmountOutMin) external payable {
    ...
    function beefIn (address yieldhubVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
    ...
    function beefOut (address yieldhubVault, uint256 withdrawAmount) external {
        (IYieldHubVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(yieldhubVault);
    ...
    function beefOutAndSwap(address yieldhubVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (IYieldHubVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(yieldhubVault);
        address token0 = pair.token0();
        address token1 = pair.token1();
        require(token0 == desiredToken || token1 == desiredToken, "YieldHub: desired token not present in liqudity pair");
    ...
    function _getVaultPair (address yieldhubVault) private view returns (IYieldHubVaultV6 vault, IUniswapV2Pair pair) {
        vault = IYieldHubVaultV6(yieldhubVault);
    ...
    function _swapAndStake(address yieldhubVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        require(isInputA || pair.token1() == tokenIn, "YieldHub: Input token not present in liqudity pair");
    ...
    function estimateSwap(address yieldhubVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        checkWETH();
        (, IUniswapV2Pair pair) = _getVaultPair(yieldhubVault);
    ...
    function checkWETH() public view returns (bool isValid) {
        ...
        require(isValid, "YieldHub: WETH address not matching Router.WETH()");
    }
